package org.jeffgrubb.NJTrains.model.stops.support;

import org.jeffgrubb.NJTrains.Connection;
import org.jeffgrubb.NJTrains.model.DaoBase;
import org.jeffgrubb.NJTrains.model.stops.Stop;
import org.jeffgrubb.NJTrains.model.stops.StopsDao;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStream;
import java.io.InputStreamReader;
import java.sql.PreparedStatement;
import java.sql.SQLException;
import java.util.ArrayList;

/**
 * Created by jgrubb on 1/10/16.
 */
public class StopsDaoImpl extends DaoBase implements StopsDao {

    public StopsDaoImpl(Connection connection) { super(connection); }

    public ArrayList<Stop> getStopsFromCsv(InputStream stream) {

        InputStreamReader is = new InputStreamReader(stream);
        BufferedReader reader = new BufferedReader(is);

        ArrayList<Stop> retValue = new ArrayList<Stop>();

        try {
            int cnt = 0;
            for(String line; (line = reader.readLine()) != null; ) {

                // ignore first line
                if(cnt++ == 0) continue;

                String[] tokens = line.split("[,]");

                if(tokens.length != 7) {
                    System.out.println("Invalid input reading stops from file. Length: " + tokens.length);
                }

                int i = 0;

                Stop stop = new Stop();
                stop.setId(Integer.parseInt(tokens[i++]));
                stop.setCode(Integer.parseInt(tokens[i++]));
                stop.setName(tokens[i++].replace("\"", ""));
                stop.setDescription(tokens[i++]);
                stop.setLatitude(Double.parseDouble(tokens[i++]));
                stop.setLongitude(Double.parseDouble(tokens[i++]));
                stop.setZoneId(Integer.parseInt(tokens[i++]));

                retValue.add(stop);
            }
        }
        catch(IOException ex) {
            System.out.println("Caught IOException while reading agencies from file. Exception: " + ex.toString());
            ex.printStackTrace();
            retValue.clear();
        }

        return retValue;
    }

    public void saveStop(Stop stop) {
        try {
            PreparedStatement ps = this.connection.prepareStatement("insert into agency\n" +
                    "(agency_id, agency_name, agency_url, agency_timezone, agency_lang, agency_phone)\n" +
                    "values (?, ?, ?, ?, ?, ?)");

            int i = 1;

            /*ps.setString(i++, agency.getId());
            ps.setString(i++, agency.getName());
            ps.setString(i++, agency.getUrl());
            ps.setString(i++, agency.getTimezone());
            ps.setString(i++, agency.getLang());
            ps.setString(i++, agency.getPhone());*/

            ps.executeUpdate();

        } catch(SQLException ex) {
            System.out.println("Caught SQLException trying to insert agency into the database. Exception: " + ex.toString());
            ex.printStackTrace();
        }
    }

    @Override
    protected String getTableName() {
        return "stops";
    }

    @Override
    protected String getTableDefinition() {
        return super.getTableDef("stops.sql");
    }
}